...
/*
 * CODE FROM https://gist.github.com/jawinn/f466b237c0cdc5f92d96 =)
 */

namespace Kinetic.Utils
{
    ...
    void Awake()
    {
        ...
        if (_motor == null) { Debug.LogError("GroundChecker did not find a CharacterController component."); }
    }
    ...
    void FixedUpdate()
    {
        ...
        if (_motor && _motor.GroundingStatus.IsStableOnGround) {
            CheckGround(new Vector3(transform.position.x, transform.position.y - (_motor.Capsule.height / 2) + startDistanceFromBottom, transform.position.z));
        }
    }
    ...
    void OnDrawGizmosSelected()
    {
        if (showDebug)
        {
            ...
            Vector3 startPoint = new Vector3(transform.position.x, transform.position.y - (_motor.Capsule.height / 2) + startDistanceFromBottom, transform.position.z);
            Vector3 endPoint = new Vector3(transform.position.x, transform.position.y - (_motor.Capsule.height / 2) + startDistanceFromBottom - sphereCastDistance, transform.position.z);
