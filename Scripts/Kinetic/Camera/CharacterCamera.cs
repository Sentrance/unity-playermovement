...
namespace Kinetic.Camera
{
    public class CharacterCamera : MonoBehaviour
    {
        [Header("Framing")] public UnityEngine.Camera Camera;
        ...
        {
            PlanarDirection = lookAt;
        }

        public void UpdateWithInput(float deltaTime, float zoomInput, Vector3 rotationInput, bool perfectFollow)
        {
            if (FollowTransform)
            {
                if (InvertX)
                ...
                if (perfectFollow)
                {
                    PlanarDirection = Vector3.Cross(FollowTransform.up,
                        Vector3.Cross(PlanarDirection, FollowTransform.up));
                    planarRot = Quaternion.LookRotation(PlanarDirection, FollowTransform.up);
                }
                else
                {
                    PlanarDirection = Vector3.Cross(Vector3.up,
                        Vector3.Cross(PlanarDirection, Vector3.up));
